...
public class RuneKnightSkill extends Skill{
    public RuneKnightSkill(String name, String description, double multiplier, int energyCost, DamageType damageType, TargetType targetType){
    ...
    public int execute(Entity caster) {
        if (caster instanceof Entities.Characters.Character) {
            ...
            if(Math.random() <= 0.6){
                ...
                delay(1000);
                caster.setHealth(caster.getMaxHealth() * 15 + caster.getHealth());
                if(caster.getHealth() > caster.getMaxHealth()) caster.setHealth(caster.getMaxHealth());
                return (int)(baseDamage * multiplier * 1.5);
            }else{
